...
for (( i = $daycount_s; i>=$daycount_e; i-- ));do
	...
	##导入订单数据
	echo "导入/home/boss/shell/hive/common_stat/movie_income_stat/data/letv_vip_order_$date.txt到${bosstdyIp}的T_NEW_ORDER_DAY表中";
        orderDayFile=/home/boss/shell/hive/common_stat/movie_income_stat/data/letv_vip_order_$date.txt
	...
	mysql --default-character-set=utf8  -P 3829 -h $bosstdyIp -u $bosstdyUser -p$bosstdyPassword $bosstdyDatebase -s -N --local-infile=1 -e "truncate T_NEW_ORDER_DAY;load data local infile '$orderDayFile' IGNORE into table T_NEW_ORDER_DAY fields terminated by '\t' IGNORE 1 lines;"
        rm -rf $orderDayFile

        ##导入播放数据
        echo "load play data into bosstdy! date=$date";
        mysql --default-character-set=utf8  -P 3829 -h $bosstdyIp -u $bosstdyUser -p$bosstdyPassword $bosstdyDatebase -s -N --local-infile=1 -e "set @date='$date';delete from PAY_MOVIE_PLAY where date=@date; load data local infile '/home/boss/shell/hive/common_stat/movie_income_stat/data/movie_income_pc.$date' IGNORE into table PAY_MOVIE_PLAY  fields terminated by '\t' (playlistId,videoId,userid,terminal,playtype,date);"
...
echo "导入PC端影片收入明细结束，时间范围${startDate} ~ ${endDate}";
