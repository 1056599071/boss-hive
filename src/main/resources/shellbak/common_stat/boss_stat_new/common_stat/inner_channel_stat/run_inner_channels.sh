...
(select t.uid,t.letv_cookie,t.cur_url_ref,t.dt from
   (select uid,letv_cookie,filter_ref(cur_url) as cur_url_ref,dt from data_raw.tbl_pv_hour where  dt>='${yesterday}' and dt<'${today}' and product='1')t 
     where t.cur_url_ref is not null)tt1
left outer join
(select t2.uid,t2.letv_cookie,t2.cur_url_ref,t1.neworxufei,t2.dt from
(select userid,status,neworxufei,dt from data_raw.t_new_order_4_data where dt>='${yesterday}' and dt<'${today}' and orderpaytype!='-1' and status='1' and terminal='112' and ordertype not in (0,1,1001))t1
join
(select uid,letv_cookie,filter_ref(cur_url) as cur_url_ref,dt from data_raw.tbl_pv_hour where dt>='${yesterday}' and dt<'${today}' and product='1' and ilu='0' and cur_url!='-' and cur_url!='' and cur_url is not null)t2
...
group by tt1.dt,tt1.cur_url_ref,tt2.neworxufei" > ./data/inner_channels_income.$yesterday
