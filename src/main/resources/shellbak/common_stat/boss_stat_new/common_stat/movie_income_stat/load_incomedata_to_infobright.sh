...
for (( i = $daycount_s; i>=$daycount_e; i-- ));do
        ...
        mysql --default-character-set=utf8  -P 3307 -h $infobrightIp -u $infobrightUser -p$infobrightPassword $infobrightDatebase -s -N --local-infile=1 -e "load data local infile '/home/boss/shell/hive/common_stat/boss_movie_income/data/movie_income_pc.$date' IGNORE into table PAY_MOVIE_PLAY  fields terminated by '\t' (playlistId,videoId,userid,terminal,playtype,date);"
