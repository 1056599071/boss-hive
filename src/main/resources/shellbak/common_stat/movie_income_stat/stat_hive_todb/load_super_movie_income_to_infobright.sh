...
while [[ ${date} < ${endDate} ]]
do
    echo "导入/home/boss/shell/hive/common_stat/movie_income_stat/data/movie_income_super.$date到${infobrightIp}的PAY_MOVIE_PLAY表中"
    mysql --default-character-set=utf8 -P 3307 -h ${infobrightIp} -u ${infobrightUser} -p${infobrightPassword} ${infobrightDatebase} -s -N --local-infile=1 -e "load data local infile '/home/boss/shell/hive/common_stat/movie_income_stat/data/movie_income_super.$date' IGNORE into table PAY_MOVIE_PLAY fields terminated by '\t' (playlistId,videoId,userid,terminal,playtype,date);"
...
pwd='/home/boss/shell/hive/common_stat/movie_income_stat/stat_hive_todb'
