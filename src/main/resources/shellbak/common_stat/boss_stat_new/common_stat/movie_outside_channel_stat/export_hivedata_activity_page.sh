...
(select userid,money,dt from data_raw.t_new_order_4_data where dt='${yesterday}' and  status='1' and money!='0' and terminal='112' and orderpaytype='2')t_order
right outer join
...
(select uid,dt,letv_cookie,parse_url(ref,'HOST') as ref,pid
    from data_raw.tbl_play_day_boss where dt='${yesterday}' and p1='1' and property like '%pay=0%' and letv_cookie!='-' and letv_cookie!='0' and letv_cookie!='')t2
