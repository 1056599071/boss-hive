...
for (( i = $daycount_s; i>=$daycount_e; i-- ));do
	...
	echo "load order data into bosstdy! date=$date";
        orderDayFile=/home/boss/shell/hive/common_stat/boss_movie_income/data/letv_vip_order_$date.txt
	...
	mysql --default-character-set=utf8  -P 3829 -h $bosstdyIp -u $bosstdyUser -p$bosstdyPassword $bosstdyDatebase -s -N --local-infile=1 -e "delete from T_NEW_ORDER_DAY;load data local infile '$orderDayFile' IGNORE into table T_NEW_ORDER_DAY fields terminated by '\t' IGNORE 1 lines;"
        ...
        mysql --default-character-set=utf8  -P 3829 -h $bosstdyIp -u $bosstdyUser -p$bosstdyPassword $bosstdyDatebase -s -N --local-infile=1 -e "load data local infile '/home/boss/shell/hive/common_stat/boss_movie_income/data/movie_income_pc.$date' IGNORE into table PAY_MOVIE_PLAY  fields terminated by '\t' (playlistId,videoId,userid,terminal,playtype,date);"
...
pwd=`pwd`
echo $pwd
pwd='/home/boss/shell/hive/common_stat/boss_movie_income/stat_hive_todb'
