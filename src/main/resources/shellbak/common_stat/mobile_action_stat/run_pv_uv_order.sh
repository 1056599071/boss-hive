...
hive -e "add jar  /home/boss/shell/hive/common_stat/mobile_action_stat/letv-boss-stat-1.2.jar;
...
FROM
	(
		...
		AND t.act_property not LIKE '%name=%'
		AND t.deviceid != '-'
		GROUP BY
			act_code,
			act_fl,
...
JOIN (
	...
	AND p.p3 = q.p3
)" > ./data/t_mobile_channel_result.${yesterday//-/}
